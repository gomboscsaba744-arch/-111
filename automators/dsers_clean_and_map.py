...
def run_dsers_clean_and_map(progress_callback=None):
    def log(msg):
        print(msg)
        if progress_callback:
            progress_callback(msg)

    log("[*] 开始执行 DSers 数据五重清洗与模板映射...")
    try:
        # ==========================
        # 阶段一：数据清洗
        # ==========================
        df = pd.read_excel(file_path, dtype=str)
        log(f"[*] 进度提示：读取原始数据 {len(df)} 条，开始五重清洗...")
        log(f"处理前数据量: {len(df)}")
        ...
        if '客户账号' in df.columns:
            ...
            mask4 = ~df['客户账号'].isin(['1000001900'] + [f'10000000{i:02d}' for i in range(1, 11)])
            mask5 = df['客户账号'].str.isdigit()
            df = df[mask1 & mask2 & mask3 & mask4 & mask5]
            log(f"客户账号过滤后剩余: {len(df)}")
        ...
        if 'SKU' in df.columns:
            ...
            df = df[mask5]
            log(f"SKU排除code后剩余: {len(df)}")

        # 覆盖保存原文件
        df.to_excel(file_path, index=False)
        log("✅ 第一阶段：数据清洗完成，已覆盖保存原文件。")
        ...
        log(f"✅ 生成 {len(df_out)} 行标准 DSers 导入表！")
        log(f"[*] 进度提示：清洗与格式映射完成，共就绪 {len(df_out)} 条有效订单")
        log("✅ 第二阶段：DSers 模板映射完成！")
        log(f"📂 生成文件: {template_path}")
        log(f"📂 生成文件: {csv_path}")
        return True
